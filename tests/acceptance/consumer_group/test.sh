...
for resource in basic with_plugins multi_plugins with_name with_labels consumer_test; do
    ID=$(tofu state show apisix_consumer_group.$resource 2>/dev/null | grep '^\s*id\s*=\|^\s*username\s*=\|^\s*rule_id\s*=\|^\s*config_id\s*=' | head -1 | sed 's/.*= *"\([^"]*\)".*/\1/')
    tofu state rm apisix_consumer_group.$resource
    tofu import apisix_consumer_group.$resource "$ID"
done
...
# Cleanup
rm -f .tofurc
echo "✓ All consumer_group tests passed"
