...
for resource in basic with_hosts with_plugins with_upstream with_labels with_script; do
    ID=$(tofu state show apisix_service.$resource 2>/dev/null | grep '^\s*id\s*=\|^\s*username\s*=\|^\s*rule_id\s*=\|^\s*config_id\s*=' | head -1 | sed 's/.*= *"\([^"]*\)".*/\1/')
    tofu state rm apisix_service.$resource
    tofu import apisix_service.$resource "$ID"
done
