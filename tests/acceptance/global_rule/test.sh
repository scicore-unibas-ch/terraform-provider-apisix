...
for resource in basic multi_plugins ip_restriction route_integration; do
    ID=$(tofu state show apisix_global_rule.$resource 2>/dev/null | grep '^\s*id\s*=\|^\s*username\s*=\|^\s*rule_id\s*=\|^\s*config_id\s*=' | head -1 | sed 's/.*= *"\([^"]*\)".*/\1/')
    tofu state rm apisix_global_rule.$resource
    tofu import apisix_global_rule.$resource "$ID"
done
...
# Cleanup
rm -f .tofurc
echo "✓ All global_rule tests passed"
