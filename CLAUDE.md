...
| **apisix_ssl** | **TODO — required for v1.0** (was item 8 of the original locked plan). Hardest of the in-plan resources because APISIX does not return the private key: Read must preserve the plan-side value rather than reconciling from API. As of APISIX 3.17 (#13227) the Admin API **strips the private-key array from GET single and list responses entirely** (previously it echoed it encrypted) — so Read gets nothing back for the key and must never expect it. Design Read to leave `private_key` as the last-applied plan value; do not diff it against the API. `Sensitive: true` on `private_key`, optional SNI parsing from PEM. README currently tells users this resource is missing. |
| apisix_stream_route | not started — easy-medium. Flat 5-field schema (`upstream_id`, `remote_addr`, `server_addr`, `server_port`, `sni`). Server-assigned ID (Computed) breaks the `id=Required` convention — decide whether to break the pattern or force users to supply IDs. Requires APISIX `stream_proxy.tcp` enabled in `tests/docker-compose.yml` for acceptance; APISIX 3.17 (#13153) adds batch TCP/UDP **port ranges** in `stream_proxy` config — a test-infra option, not a change to the stream_route object schema. |
