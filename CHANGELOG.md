...
- **`apisix_consumer_plugins` resource** — manages individual plugins on a consumer owned by another system (a portal, an IdP sync, a provisioning script) without taking ownership of the consumer or its credentials. Only the named plugin keys are written; `username`, `group_id`, `labels` and credential plugins such as `key-auth` are read and written back untouched, so an apply cannot invalidate a credential Terraform did not issue. Fills the gap between "Terraform owns the whole consumer" (`apisix_consumer`) and "no per-consumer plugin config in version control at all"; the motivating case is a per-user quota exception on top of a route-level default, which works because APISIX resolves consumer-level plugin config ahead of route-level.

  Writes are read-modify-write (`GET` → merge → `PUT`): the Admin API has no `PATCH` for `/consumers/{username}`, unlike routes, services and consumer groups. Two consequences are documented rather than hidden — an external full rewrite of the consumer (e.g. a credential rotation) drops the managed plugins until the next apply, and a single consumer must be managed by exactly one `apisix_consumer_plugins` resource, since two would race each other during a parallel apply. Import IDs are `<consumer>/<plugin>[,<plugin>...]`; importing by bare consumer name is refused so that `key-auth` cannot be pulled into state.
...
- **`make test-env-up` no longer waits a minute on a healthy stack.** Its readiness probe polled `/apisix/admin/routes` without the `X-API-KEY` header, so it got 401 on every attempt and exhausted all 30 before continuing anyway. Two further bugs in the same loop: the attempt list was `1..20 16 21..30` (31 entries, `16` twice) so the counter was wrong, and exhausting it fell through to success rather than failing. The loop had been copy-pasted into three targets and only this copy had drifted; it is now a single `wait-for-apisix` target the others depend on, with the URL, key and attempt count as overridable variables, a non-zero exit and APISIX logs on timeout.

## [0.4.0] - 2026-07-06

> Released without a CHANGELOG section at the time; recorded here retroactively
> from the `[Unreleased]` block as it stood at the `v0.4.0` tag.
